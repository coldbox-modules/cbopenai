	...
	function run( testResults, testBox ){
		describe( "OpenAIService.cfc", function(){
			beforeEach( function( currentSpec ){
				...
				openAIService.setSettings( { "apiKey" : "fake-key" } );
			} );
			...
			describe( "model", function(){
				it( "returns the model", function(){
					openAIService.$(
						...
						{
							"status_code" : 200,
							"fileContent" : serializeJSON( { "id" : "text-davinci-003", "object" : "model" } )
						}
					);
					...
					openAIService.$(
						"performHTTPRequest",
						{
							...
							"fileContent" : serializeJSON( {
								...
								"choices" : [ { "text" : "Yes, this is a test" } ]
							} )
						}
					);
			...
			describe( "chat completion", function(){
				it( "returns a chat completion", function(){
					...
					var result = openAIService.createChatCompletion(
						...
						"performHTTPRequest",
						{
							...
							"fileContent" : serializeJSON( {
								...
								"data"   : [
									{
										"object"    : "embedding",
										"embedding" : [ 0.0023434, 0.234934234 ]
									}
								]
							} )
						}
			...
			describe( "moderation", function(){
				it( "moderation with basic input calls API", function(){
					openAIService.$( "performHTTPRequest", { "status_code" : 200, "fileContent" : "{}" } );
				...
				it( "a non-200 status code throws error", function(){
					openAIService.$( "performHTTPRequest", { "status_code" : 400, "fileContent" : "{}" } );
					expect( function(){
						var result = openAIService.createModeration( input = "The cat and the hat." );
					} ).toThrow( "BadRequest" );
				} );
			...
			describe( "audio transcription", function(){
				it( "returns a generated transactions", function(){
					openAIService.$(
						...
						{
							"status_code" : 200,
							"fileContent" : serializeJSON( { "text" : "Hello there" } )
						}
					);
